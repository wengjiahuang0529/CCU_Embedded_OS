...
typedef struct {
    char command[10]; // Store the command string
} Command_t;
...
void change_color_to_white(int freq){
	PA12 = 0; // Blue component
	PA13 = 0; // Green component
	PA14 = 0; // Red component
}
...
int main(void)
{
    ...
    SYS_LockReg();
		...
		xCommandQueueA = xQueueCreate(COMMAND_QUEUE_LENGTH, sizeof(Command_t));
		xCommandQueueB = xQueueCreate(COMMAND_QUEUE_LENGTH, sizeof(Command_t));
		xCommandQueueC = xQueueCreate(COMMAND_QUEUE_LENGTH, sizeof(Command_t));
		printf("\n\n---LAB2 Bonus---\r\n");
...
void vStartThreadTasks( void )
{
		...
		xTaskCreate(TaskC, "TaskC",128,NULL,1,( xTaskHandle * ) NULL );
}
static void TaskC(void* pvParameters)
{
		int mode;
		int freq=30;
		while(1){
				if (xQueueReceive(xCommandQueueC, &mode, portMAX_DELAY))
        {
						keep_blink=1;
						printf("Blink mode in c: %d\n",mode);
						if(mode == 0){
									while(keep_blink){
										blink_left_to_right(freq);
									}
							}
						else if(mode==1){
									while(keep_blink){
										blink_right_to_left(freq);
									}
							}
				}
		}
...
static void TaskA(void* pvParameters)
{
        ...
        xQueueSend(xCommandQueueA, &cmd, portMAX_DELAY);
			
				if (strcmp(cmd.command, "LED0") == 0)
        {
						// Wait for the mode from Task B
						xQueueReceive(xCommandQueueB, &mode, portMAX_DELAY);
						printf("\nReceived Mode: %d\n", mode);
						if(mode == 0){
									xQueueSend(xCommandQueueC, &mode, portMAX_DELAY);
						}
						else if(mode==1){
									xQueueSend(xCommandQueueC, &mode, portMAX_DELAY);
						}
						else{
									printf("wrong command! please input 0 or 1");
						}
				}
				
				else if(strcmp(cmd.command, "BLU0") == 0 || strcmp(cmd.command, "GRE0") == 0 || strcmp(cmd.command, "RED0") == 0 )
				{
					// Wait for the mode from Task B
					xQueueReceive(xCommandQueueB, &white, portMAX_DELAY);
					printf("\nReceived Mode: %c\n", white);
					if(white=='y' || white=='Y'){
						printf("White!!!\n");
						change_color_to_white(freq);
					}else if(white=='n' || white=='N'){
				...
				if (xQueueReceive(xCommandQueueA, &cmd, portMAX_DELAY) == pdPASS)
        {
            ...
            if (strcmp(cmd.command, "LED0") == 0)
            {
                ...
                printf("LEDs turned ON\n");
								...
								printf("LED Mode(0/1):");
								scanf("%d",&mode);
								xQueueSend(xCommandQueueB, &mode, portMAX_DELAY);
						}				
						...
						else if(strcmp(cmd.command,"BLU0")==0)
						{
								...
								printf("Change to white or not?(y/n):");
								scanf("%c",&white);
								xQueueSend(xCommandQueueB, &white, portMAX_DELAY);
						}
						...
						/*----------------------------------------------------*/
						else if(strcmp(cmd.command,"RGB1")==0){
								printf("RGB LED off\n");
								turn_RGBLED_off();
						}
						else{
